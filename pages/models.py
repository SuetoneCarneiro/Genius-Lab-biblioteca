...
class Emprestimo(models.Model):
    ...
    status = models.CharField(
        max_length=20,
        choices=[('solicitado', 'Solicitado'), ('em_aberto',
                                                'Em Aberto'), ('concluido', 'Concluído')],
        default='solicitado',
        verbose_name='Status'
    )
    data_emprestimo = models.DateField(
        default=timezone.now(), verbose_name='Data do Empréstimo')
    data_devolucao = models.DateField(default=date.today(
    ) + timedelta(days=7), verbose_name='Data de Devolução')
    fk_usuario = models.ForeignKey(
        User, on_delete=models.CASCADE, verbose_name='Usuário')
    fk_livro = models.ForeignKey(
...
class Contem(models.Model):
    emprestimo = models.ForeignKey(
        Emprestimo, on_delete=models.CASCADE, verbose_name='Empréstimo')
    livro = models.ForeignKey(
        Livro, on_delete=models.CASCADE, verbose_name='Livro')
    quantidade_emprestada = models.PositiveIntegerField(
        verbose_name='Quantidade emprestada')
